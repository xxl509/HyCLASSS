classdef SpectralTrainClass
    ...
    methods
        ...
        function obj = computeSpectralTrain(varargin)
            ...
            if START_ANALYSIS_FOR_EACH_FILE == 1
                ...
                load([xlsFileContentCheckSummaryOut,'.mat']);
                txt = headerTable;
                raw = txt;
                ...
                check = cell2mat(txt(2:end, checkIndex));
                check = [NaN;check];
                ...
                S = regexp(checkFile,'/','split');
                load([S{end},'.mat']);
                txt = splitFileListCellwLabels;
                ...
                for f = processStart:processFiles
                    ...
                    %% Load data
                    % Define test file
                    edfFn = strcat(edfFolderList{f}, edfFileList{f});
                    figs = [];
                    ...
                    %% Load Annotations
                    xmlFn = strcat(xmlPath{f}, xmlFNames{f});
                    xmlFn = strcat(xmlFolderList{f}, xmlFileList{f});
                    
                    lcaObj = loadCompumedicsAnnotationsClass(xmlFn);
                    lcaObj.scoreKey = scoreKey;
                    lcaObj.GET_SCORED_EVENTS = 1;
                    ...
                    if COHERENCE_COMPUTE_SPECTRAL == 1
                        ...
                        % Zero Signal by 30 second epochs ��ȥ�����źŵ�ƽ��ֵ
                        dataAvg = reshape(signalCell{1}(1:numPtsPer30secEpoch*returnedNum30SecEpochs),...
                            numPtsPer30secEpoch, returnedNum30SecEpochs)';
                        dataAvg =reshape((mean(dataAvg,2)*ones(1, numPtsPer30secEpoch))', ... %mean(A,2)���ǰ�Aÿһ�е�ƽ��ֵ��������
                        ...
                        for s = 1:processNsignals
                            ...
                            dataAvg = reshape((transpose(mean(dataAvg,1))*ones(1, numPtsPer30secEpoch))', ...
                                returnedNum30SecEpochs*numPtsPer30secEpoch,1); %transpose()����ת��
                            dataZeroed = ...
                            ...
                            %   Other features I add
                            %  if SR = 128 it is EEG, if SR = 1024 it is Heart Rate
                            
                            
                            if  eval(['exist(''',StudyEdfResultDir,strrep(edfFileList{f},'edf',''),signalLabels{s},'.FeatureJin.mat'',''file'')']);
                            else
                                [featureMatrix,featureLabel] = FeaturesJinAdd(dataZeroed,signalLabels{s},epochWidth,noverlap,spectralBinWidth,SR);
                                eval(['save ',StudyEdfResultDir,strrep(edfFileList{f},'edf',''),signalLabels{s},'.FeatureJin.mat featureMatrix featureLabel;']);
                                
                            end
                            
                            
                            clear featureMatrix featureLabel;
                        end
                        ...
                        if COMPUTE_TOTAL_POWER == 1
                            ...
                            for s = 1:processNsignals
                                ...
                                %Statistics of the features extracted by Jin
                                try
                                    eval(['load(''',StudyEdfResultDir,strrep(edfFileList{f},'edf',''),signalLabels{s},'.FeatureJin.mat'');']);
                                    [FtotalSleep,FtotalNREM,FtotalREM] = compute_total_power(featureMatrix,sleepMask,nremMask,remMask);
                                    if strfind(signalLabels{s},'C')
                                        totalSummary (3+s,20:19+33) = num2cell([FtotalSleep',FtotalNREM',FtotalREM']); % totally 33 features
                                    elseif strfind(signalLabels{s},'HRate')
                                        totalSummary (3+s,20:19+84*3) = num2cell([FtotalSleep',FtotalNREM',FtotalREM']); % totally 84*3 features
                                    end
                                    clear featureMatrix;
                                catch
                                    disp('ERROR. in ''SpectralTrainClass.m''. in line 1151');
                                end
                            end
                            
                            % Add labels to table
                            totalSummary(1,[1,2,8,14]) = ...
                                {edfFNames{f}, 'Sleep','NREM','REM'};
                            if strfind(signalLabels{s},'C')
                                totalSummary(1,[20,31,42]) = {'Sleep','NREM','REM'};
                            elseif strfind(signalLabels{s},'HRate')
                                totalSummary(1,[20,103,186]) = {'Sleep','NREM','REM'};
                            end
                            ...
                            if strfind(signalLabels{s},'C')
                                totalSummary(2,20:52) = [featureLabel',featureLabel',featureLabel'];
                            elseif strfind(signalLabels{s},'HRate')
                                totalSummary(2,20:20+84*3-1) = [featureLabel,featureLabel,featureLabel];
                            end
                            ...
                            if obj.EXPORT_TOTAL_POWER == 1
                                ...
                                S = regexp(totalPowerFn,'/','split');
                                eval(['save ',S{end},'.mat totalSummary;']);
                                % Echo Status to console
                        ...
                        if EXPORT_SPECTRAL_DETAILS == 1
                            ...
                            for s = 1:processNsignals
                                ...
                                S = regexp(specDetailXlsFn,'/','split');
                                eval(['save ',S{end},'.mat spectralOut;']);
                                fprintf('\t\t%.0f. %s\n', s, specDetailXlsFn);
                            end
                        end
                        ...
                        if SUMMARIZE_BANDS == 1
                            ...
                            if obj.EXPORT_INDIVIDUAL_BAND_SUMMARY == 1
                                ...
                                S = regexp(bandSumPathFn,'/','split');
                                eval(['save ',S{end},'.mat bandSumCell;']);
                                % Echo status to console
                    ...
                    if COHERENCE_COMPUTE_COHERENCE == 1
                        ...
                        S = regexp(coherenceResultFN,'/','split');
                        eval(['save ',S{end},'.mat avgCohOut;']);
                        % Save Figures
                    ...
                    eval(['save ',edfFNames{f},'.mat P artifactCell bandSubjectCell bandSumCell pxxCell spectralOut spectrumSubjectCell totalSummary featureLabel sleepMask nremMask remMask wakeMask characterHypnogram numericHypnogram signalLabels avgCohOut CxyEpochMatrix freq fIdx returnedNum30SecEpochs lcaObj analysisEegUnits;']);
                end % For each file
                ...
                if and(EXPORT_BAND_SUMMARY == 1, SUMMARIZE_BANDS == 1)
                    ...
                    S = regexp(bandFile,'/','split');
                    eval(['save ',S{end},'.mat bandSubjectSummaryCell;']);
                end
                ...
                if and(OUTPUT_AVERAGE_SPECTROGRAMS == 1, PLOT_NREM_REM_SPECTRUM == 1)
                    ...
                    S = regexp(spectralFile,'/','split');
                    eval(['save ',S{end},'.mat spectrumSubjectCell;']);
                end
                
            end
            save allResultsInComputeSepectralTrain024_030  bandSubjectSummaryCell bandTable  spectrumSubjectCell;
    ...
    methods(Static)
        ...
        function CreateSignalPPT (figs, pptFn, titleStr, imageResolution)
            ...
            S = regexp(pptFn,'/','split');
            eval(['save ',S{end},'.mat imageResolution titleStr figs;']);
            
            hgsave(figs,strcat(S{end},'.fig'));
            %             if ~isempty(figs)
