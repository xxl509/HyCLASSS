function [ACTIVITY, MOBILITY, COMPLEXITY,m0,m1,m2] = F_hjorth(S,UC,A)
...
%   in G. Dolce, H. Kunkel: CEAN Computerized EEG Analysis, Gustav Fischer
%   1975, S.3-8.
...
ACTIVITY   = sqrt(m0);
if isnan(ACTIVITY)
    disp(['Feature of ACTIVITY is NaN. ']);
...
if isnan(COMPLEXITY)
    disp(['Feature of COMPLEXITY is NaN. ']);
   
end
MOBILITY   = sqrt(MOBILITY);
if isnan(MOBILITY)
    disp(['Feature of MOBILITY is NaN. ']);
