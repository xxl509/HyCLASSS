classdef BlockEdfLoadClass
    ...
    % 
    %    Bob Kemp, Alpo V鋜ri, Agostinho C. Rosa, Kim D. Nielsen and John Gade 
    ...
    %       recording_startdate
    %       recording_starttime
    %       num_header_bytes % 包含了signalHeader 和Header 的bytes总的大小
    %       reserve_1
    %       num_data_records
    ...
    methods
        ...
        function varargout = blockEdfLoad(obj, varargin)
            ...
            % Check that number of arguments and epochs are right
            ...
            try
                ...
                edfHeaderSize = 256; % 256在sleepdata中是确定的
            ...
            headerVariablesConF = {...
                ...
                @str2num};
            headerVariableSize = [ 8; 80; 80; 8; 8; 8; 44; 8; 8; 4]; %每个field 分配的bytes大小
            headerVarLoc = vertcat([0],cumsum(headerVariableSize));
            headerSize = sum(headerVariableSize);

            % Create Header Structure 从已知的(输入的)class obj复制一个新的
            header = struct();
            ...
            if numCompToLoad >= 2
                try 
                    ...
                    [A count] = fread(fid, edfSignalHeaderSize); % reads sizeA elements into A and positions the file pointer after the last element read
                catch exception
                ...
                for v = 1:num_signal_header_vars %把所有signals的第v个header field取出来
                    varBlock = A(signalHeaderVarLoc(v)+1:signalHeaderVarLoc(v+1))';
                    varSize = signalHeaderVarSize(v);
                    conF = signalHeaderVarConvF{v};
                    for s = 1:num_signals %再将第v个header分别放再第s个signal header中
            ...
            if numCompToLoad >=3
                ...
                try
                    ...
                    if obj.num_argsin == 1 %函数输入的参数只有一个
                        % Load entire file
                        ...
                        edfRecordSize = sum(edfSignalSizes);% the sum of all sampling rates
                        ...
                        for r = 1:header.num_data_records %1秒1秒读，先取出所有28signals的每一秒的数据，再分28signals
                ...
                for s = 1:num_signals
                    ...
                    % Convert to analog signal 转换成模拟信号
    ...
    methods(Static)
        ...
        function varargout = GetEdfFileListInfo(varargin)
            ...
            %---------------------------------------------- FlattenFileTree
            function varargout = flattenFileTree(fileTree, folder) %静态方法中的内嵌方法
